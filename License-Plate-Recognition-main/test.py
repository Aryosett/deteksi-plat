...
# Ground truth (plat nomor yang benar) untuk evaluasi akurasi
ground_truth = "XX1234ABC"  # Gantilah dengan plat nomor yang benar sesuai data Anda
...
def process_plate(plate):
    ...
    # Peningkatan kontras (penajaman gambar)
    sharpen_kernel = np.array([[0, -1, 0], [-1, 5,-1], [0, -1, 0]])  # Kernel untuk penajaman
    plate_sharpened = cv2.filter2D(plate_morph, -1, sharpen_kernel)
    save_image(plate_sharpened, "result_1_sharpened.png")

    return plate_resized, plate_gray, plate_thresh, plate_morph, plate_sharpened
...
def calculate_accuracy(predicted_text, ground_truth):
    """
    Menghitung akurasi dengan membandingkan teks hasil OCR dengan ground truth.
    """
    # Menghitung jumlah karakter yang cocok
    correct_chars = sum(1 for a, b in zip(predicted_text, ground_truth) if a == b)
    accuracy = (correct_chars / len(ground_truth)) * 100
    return accuracy
...
def detect_plate_realtime():
    ...
    while True:
        ...
        if key == ord('s') and detected_plate is not None:
            ...
            plate_resized, plate_gray, plate_thresh, plate_morph, plate_sharpened = process_plate(detected_plate)

            # OCR untuk membaca teks dari plat nomor
            text = extract_text_from_plate(plate_sharpened)
