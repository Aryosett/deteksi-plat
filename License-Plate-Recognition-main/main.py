...
def save_image(image, filename):
    """
    Simpan gambar dengan nama file yang diberikan.
    """
...
def process_plate(plate):
    ...
    save_image(plate_resized, "Result_Image.png")

    # Konversi ke grayscale
    plate_gray = cv2.cvtColor(plate_resized, cv2.COLOR_BGR2GRAY)
    save_image(plate_gray, "Result_Image.png")
...
def detect_plate_realtime():
    ...
    while True:
        ...
        # Jika 's' ditekan, hapus semua file gambar di folder dan simpan gambar baru
        if key == ord('s') and detected_plate is not None:
            # Hapus semua file lama di folder hasil
            delete_all_files_in_folder(output_folder)
            
            # Simpan hasil crop plat nomor
            save_image(detected_plate, "Detected_Plate.png")
            print(f"Plat nomor disimpan sebagai 'Detected_Plate.png'.")
            ...
            save_image(frame, "Result.png")
            print(f"Hasil deteksi disimpan sebagai 'Result.png'.")
